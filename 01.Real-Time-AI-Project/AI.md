...
- Will install Python
- Download [Gitbash](https://git-scm.com/install/windows)
- Open IDE
...
- Multiple Agentic AI exists and we will use *Strands Agents* [Open-Source by AWS]
...
official Guide to install [Strandsagents](https://strandsagents.com/docs/user-guide/quickstart/python/)

- To install tools	[Open Google and search "strandsagents.com/latest"]
	* pip install strands-agents-tools
	
  ![alt text](image-4.png)

- To build new agents
- In IDE, we will create a new file called "agent.py"
	* agent.py	
	```powershell
  # from strands import Agent
  # agent = Agent() # Create an instance of the Agent class
  # # By default, it runs Amazon Bedrock in the Background, which provides access to various language models.
  # # the agent will use the 'gpt-3.5-turbo' model, but you can specify a different model if needed
  ...
  # Use the agent
  # agent("How are you") # Prints model output to stdout by default
  user_input = input("Enter your question: ")
  agent(user_input)
  ```
	
- To install local LLM agent [ollama](https://strandsagents.com/docs/user-guide/concepts/model-providers/ollama/)
	* pip install 'strands-agents[ollama]'
![alt text](image-5.png)

Download [ollama on windows Machine](https://ollama.com/download/windows)

- [Youtube Link for install ollama](https://www.youtube.com/watch?v=XTXEbVVdde4&t=53s) 
- [Youtube Link for install ollama](https://www.youtube.com/watch?v=zKmb3QrWoJ4)
...
Note:- If you ask for current time and date then it won't reply you.
![alt text](image-6.png)

We need to modify the ```agent.py``` code like below
```python
  # from strands import Agent
  ...
  system_prompt = "You are a helpful assistant that provides concise and accurate answers to user questions. You can use tools wherever necessary to gather information or perform tasks and make API calls for free API that doesn't need API keys. Always ensure your responses are clear and to the point. "

  # Create an agent using the Ollama model
  agent = Agent(model=ollama_model, 
                system_prompt=system_prompt, 
                tools=[http_request])  # Include the HTTP request tool for making API calls if needed

  # agent = Agent(tools=[http_request])  # Include the HTTP request tool for making API calls if needed
...
Need to [clone the code](https://github.com/LondheShubham153/strands-temporal-agents.git)


[Install Docker on Windows Server 2025](https://www.youtube.com/watch?v=Q6JVnkg-Mz4)

- https://www.configserverfirewall.com/docker/install-docker-windows-server-2025/


- install visual studio code on ubuntu
```bash
sudo dpkg -i code_1...xxxxxxxxxxxxxxx.deb
```
- [How to Install and Use Visual Studio Code on Ubuntu Linux (VS Code)](https://www.youtube.com/watch?v=ChwsFldra-o)
...
```bash
(venv) dc-ops@docker-svr:~/AI-PROJECT$
```
...
```
Start the temporal in another tab
```bash
temporal server start-dev
```

```bash
(venv) dc-ops@docker-svr:~/AI-PROJECT/strands-temporal-agents$ pwd
/home/dc-ops/AI-PROJECT/strands-temporal-agents
(venv) dc-ops@docker-svr:~/AI-PROJECT/strands-temporal-agents$ pip install -r requirements.txt
```
```bash
(venv) dc-ops@docker-svr:~/AI-PROJECT/strands-temporal-agents$ cd docker_monitor/
(venv) dc-ops@docker-svr:~/AI-PROJECT/strands-temporal-agents/docker_monitor$ pwd
/home/dc-ops/AI-PROJECT/strands-temporal-agents/docker_monitor
(venv) dc-ops@docker-svr:~/AI-PROJECT/strands-temporal-agents/docker_monitor$ ls
docker_agent.py  docker_client.py  docker-compose.demo.yml  docker_temporal_agent.py  docker_utils.py  docs  __init__.py  README.md  test_docker_agent_basic.py  validate_docker_monitor.py
(venv) dc-ops@docker-svr:~/AI-PROJECT/strands-temporal-agents/docker_monitor$
```
